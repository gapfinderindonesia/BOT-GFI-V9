...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'FN:Dandi Saputra\n' // NAMA OWNER BOT
            + 'ORG:Creator Dandi Saputra;\n' // KASIH CREDITS GUA DANDI SAPUTRA SU!!!
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6281352445558@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6281352445558@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6281352445558@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			case 'owner':
				...
				case 'fake':          
                    ...
                    break
                case 'fakereplay':
                   client.reply(from, 'ange mas', 'mending lari', "0813-5244-5558")
                   break
                case 'but':
				...
				case 'hidetag':
					...
					if (!text) return client.sendMessage(from, '𝐌𝐚𝐬𝐮𝐤𝐚𝐧 𝐓𝐞𝐤𝐬/𝐔𝐫𝐥 𝐘𝐚𝐧𝐠 𝐈𝐧𝐠𝐢𝐧 𝐃𝐢 𝐁𝐮𝐚𝐭 𝐐𝐑', tex, {quoted: mek})
               ...
               if (anu.error) return reply(anu.error)
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 BOT GAP FINDER INDONESIA 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa?')
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Udah aktif njing')
						welkom.push(from)
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag cok')
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner Saya!', id)
